...
$sid=0; $no=0; $total_rp=0; $total=0; $pembayaran=0;

if ($_GET['module']=='home'){
    echo "<center>
		<div class='post_title'><h1>Selamat Datang di Menu Kasir Sistem Point of Sales<h1></div>
          <div class='text_area'>Terdapat beberapa fitur kasir yang dapat digunakan</div>
		  <img src='../view/images/kasir.png'></center>
		  ";
}
...
elseif ($_GET['module']=='keranjangbelanja'){
  echo "<div class='post_title'><b>Ini adalah daftar barang yang sudah anda Order.</b></div><br />
			 ...
			 <form method='GET' action='aksi.php' onSubmit='return validasi(this)''>
				...
				<input type=button value='Cari Produk' onclick=\"window.location.href='semua-produk.html';\">
		  </form>";
  ...
  $ketemu=mysql_num_rows($sql);
    ...
    echo "<form method=post action=aksi.php?module=keranjang&act=update>
          ...
          <tr style='color:#fff;  height:35px;' bgcolor=#000>
					<th>Kode</th><th>Nama Produk</th><th>Jumlah</th>
          <th>Harga</th><th>Sub Total</th><th></th>
					</tr>";
  ...
  while($r=mysql_fetch_array($sql)){
	...
	$subtotal    = $h * $r['jumlah'];
	$subtotaldiskon = $subtotal * $r['diskon']*0.01;
    ...
    echo "<tr bgcolor=$warna><input type=hidden name=id[$no] value=$r[id_orders_temp]>
              ...
              <td>Rp $subtotal_rp</td>
							...
							<input type='hidden' name='total_harga_item' value='".$subtotal_rp."'/>
  ...
  echo "<tr>
		...
		<tr>
			<td colspan=2><br />
			<input class='belanja' style='height:36px; width:150px; margin-left:9px;'
			onclick = 'getConfirmation();' type=submit value='Update' border=0><br /><br>
			<input type=button value='Ingin Belanja Lagi? Klik Disini' onclick=\"window.location.href='semua-produk.html';\">
			</td>
...
mysql_query("INSERT INTO orders(nama_kustomer, tgl_order, jam_order)
             VALUES('$_SESSION[namalengkap]','$tgl_skrg','$jam_skrg' )");
...
for ($i = 0; $i < $jml; $i++)
{
  ...
  mysql_query("INSERT INTO orders_detail(id_orders, id_produk, jumlah, cara_pembayaran,total_bayar)
               VALUES('$id_orders',{$isikeranjang[$i]['id_produk']}, {$isikeranjang[$i]['jumlah']},
								 '{$isikeranjang[$i]['cara_pembayaran']}', {$isikeranjang[$i]['total_bayar']} )");
	mysql_query("UPDATE orders SET total_bayar={$isikeranjang[$i]['total_bayar']}
